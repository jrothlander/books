...
#define NUM_PATCH_TRIANGLES (2 * (NUM_PATCH_VERTICES_U - 1)*(NUM_PATCH_VERTICES_V - 1))
#define NUM_TOTAL_VERTICES (NUM_U_POINTS + NUM_V_POINTS + NUM_PATCH_VERTICES_U * NUM_PATCH_VERTICES_V)

#define NUM_GRID_INDICES  ((2 * (NUM_U_POINTS - 1)) + (2 * (NUM_V_POINTS - 1)))
#define NUM_PATCH_INDICES (6 * NUM_PATCH_VERTICES_U * NUM_PATCH_VERTICES_V)
...
{	
	...
	for (long Index = 0; Index < NUM_U_POINTS + NUM_V_POINTS; Index++)
	{
		*pIndices       = Index;
		*(pIndices + 1) = Index + 1;
	...
	for (long UIndex = 0; UIndex < NUM_PATCH_VERTICES_U - 1; UIndex++)
	{
		for (long VIndex = 0; VIndex < NUM_PATCH_VERTICES_V - 1; VIndex++)
		{
			*pIndices       = (NUM_U_POINTS + NUM_V_POINTS) + VIndex + (UIndex * NUM_PATCH_VERTICES_V);
			*(pIndices + 1) = (NUM_U_POINTS + NUM_V_POINTS) + VIndex + (UIndex * NUM_PATCH_VERTICES_V) + 1;
			*(pIndices + 2) = (NUM_U_POINTS + NUM_V_POINTS) + VIndex + ((UIndex + 1) * NUM_PATCH_VERTICES_V);
			*(pIndices + 3) = (NUM_U_POINTS + NUM_V_POINTS) + VIndex + (UIndex * NUM_PATCH_VERTICES_V) + 1;
			*(pIndices + 4) = (NUM_U_POINTS + NUM_V_POINTS) + VIndex + ((UIndex + 1) * NUM_PATCH_VERTICES_V) + 1;
			*(pIndices + 5) = (NUM_U_POINTS + NUM_V_POINTS) + VIndex + ((UIndex + 1) * NUM_PATCH_VERTICES_V);
...
void CNURBSPatchApplication::Render()
{
	...
	m_pD3DDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0,
		                               NUM_PATCH_VERTICES_U * NUM_PATCH_VERTICES_V,
...
void CNURBSPatchApplication::DefineBasisFunctions()
{
	...
	memset(m_DerivativeBasisU, 0, NUM_PATCH_VERTICES_U * NUM_U_POINTS * PATCH_ORDER_U * sizeof(float));
	memset(m_DerivativeBasisV, 0, NUM_PATCH_VERTICES_V * NUM_V_POINTS * PATCH_ORDER_V * sizeof(float));

	//Fill the U values
	for (long Vertex = 0; Vertex < NUM_PATCH_VERTICES_U; Vertex++)
	{
		float u = (float)Vertex / (float)(NUM_PATCH_VERTICES_U - 1);
	...
	//Fill the V values
	for (Vertex = 0; Vertex < NUM_PATCH_VERTICES_V; Vertex++)
	{
		float v = (float)Vertex / (float)(NUM_PATCH_VERTICES_V - 1);
	...
	for (Order = 1; Order < PATCH_ORDER_V; Order++)
	{
		...
		{
			//Then loop by parameter value...
			for (long Vertex = 0; Vertex < NUM_PATCH_VERTICES_V; Vertex++)
			{
				//Obviously, make sure this is the same as above...
				float v = (float)Vertex / (float)(NUM_PATCH_VERTICES_V - 1);
...
BOOL CNURBSPatchApplication::FillPatchBuffer()
{
	...
	{
		pVertices[NUM_U_POINTS + V].x = 250.0f + 100.0f * cosf((float)V / (float)NUM_V_POINTS * 8.0f * D3DX_PI);
		pVertices[NUM_U_POINTS + V].y = (float)V * 400.0f / (float)(NUM_V_POINTS - 1) * (0.5f + 0.25f * ANIMATE(1.0f));
		pVertices[NUM_U_POINTS + V].z = 250.0f + 100.0f * sinf((float)V / (float)NUM_V_POINTS * 8.0f * D3DX_PI);
		pVertices[NUM_U_POINTS + V].color = 0xFF00FF00;
	}

	//Used to compute lighting
	D3DXVECTOR3 PositionU;
	D3DXVECTOR3 PositionV;
	D3DXVECTOR3 dPdU;
	D3DXVECTOR3 dPdV;
	D3DXVECTOR3 Normal;
	...
	for (U = 0; U < NUM_PATCH_VERTICES_U; U++)
	{
		for (long V = 0; V < NUM_PATCH_VERTICES_V; V++)
		{
			long Current = (NUM_U_POINTS + NUM_V_POINTS) + (U * NUM_PATCH_VERTICES_V) + V;
			...
			memset(&VOffset, 0, sizeof(D3DXVECTOR3));

			float CurrentU = (float)U / (float)(NUM_PATCH_VERTICES_U - 1);
			float CurrentV = (float)V / (float)(NUM_PATCH_VERTICES_V - 1);

			//force the last compare to work
			if (CurrentU == 1.0f) CurrentU = 1.0f - EPSILON;
			if (CurrentV == 1.0f) CurrentV = 1.0f - EPSILON;

			float NumeratorU      = 0.0f;
			float DenominatorU    = 0.0f;
			float NumeratorV      = 0.0f;
			float DenominatorV    = 0.0f;
			float SumDerivativesU = 0.0f;
			float SumDerivativesV = 0.0f;
			...
			for (long UStep = 0; UStep < NUM_U_POINTS; UStep++)
			{
				for (long VStep = 0; VStep < NUM_V_POINTS; VStep++)
				{
					...
					{
						SumDerivativesU += (m_WeightsU[UStep] * m_DerivativeBasisU[U][UStep][PATCH_ORDER_U - 1]);
						SumDerivativesV += (m_WeightsV[VStep] * m_DerivativeBasisV[V][VStep][PATCH_ORDER_V - 1]);

						//position denominators
						DenominatorU += m_WeightsU[UStep] * m_BasisFunctionsU[U][UStep][PATCH_ORDER_U - 1];
						DenominatorV += m_WeightsV[VStep] * m_BasisFunctionsV[V][VStep][PATCH_ORDER_V - 1];
			...
			//loop through once to set up the NURBS values
			for (UStep = 0; UStep < NUM_U_POINTS; UStep++)
			{
				for (long VStep = 0; VStep < NUM_V_POINTS; VStep++)
				{
					if (m_KnotVectorU[UStep] <= CurrentU && CurrentU <= m_KnotVectorU[UStep + PATCH_ORDER_U] &&
						m_KnotVectorV[VStep] <= CurrentV && CurrentV <= m_KnotVectorV[VStep + PATCH_ORDER_V])
					{
						...
						NumeratorU = (m_WeightsU[UStep] * m_BasisFunctionsU[U][UStep][PATCH_ORDER_U - 1]);
						NumeratorV = (m_WeightsV[VStep] * m_BasisFunctionsV[V][VStep][PATCH_ORDER_V - 1]);

						long ControlIndexU = UStep;
						long ControlIndexV = NUM_U_POINTS + VStep;
						...
						pVertices[Current].y += (NumeratorU * pVertices[ControlIndexU].y / DenominatorU);
						VOffset.y += (NumeratorV * pVertices[ControlIndexV].y / DenominatorV);
						pVertices[Current].z += (NumeratorU * pVertices[ControlIndexU].z / DenominatorU);
						VOffset.z += (NumeratorV * pVertices[ControlIndexV].z / DenominatorV);

						dPdU.x += pVertices[ControlIndexU].x * ((m_WeightsU[UStep] * 
							      m_DerivativeBasisU[U][UStep][PATCH_ORDER_U - 1] / DenominatorU) -
								  NumeratorU * SumDerivativesU / (DenominatorU * DenominatorU));
						dPdU.y += pVertices[ControlIndexU].y * ((m_WeightsU[UStep] * 
							      m_DerivativeBasisU[U][UStep][PATCH_ORDER_U - 1] / DenominatorU) -
								  NumeratorU * SumDerivativesU / (DenominatorU * DenominatorU));
						dPdU.z += pVertices[ControlIndexU].z * ((m_WeightsU[UStep] *
							      m_DerivativeBasisU[U][UStep][PATCH_ORDER_U - 1] / DenominatorU) -
								  NumeratorU * SumDerivativesU / (DenominatorU * DenominatorU));

						dPdV.x += pVertices[ControlIndexV].x * ((m_WeightsV[VStep] * 
							      m_DerivativeBasisV[V][VStep][PATCH_ORDER_V - 1] / DenominatorV) -
								  NumeratorV * SumDerivativesV / (DenominatorV * DenominatorV));
						dPdV.y += pVertices[ControlIndexV].y * ((m_WeightsV[VStep] * 
							      m_DerivativeBasisV[V][VStep][PATCH_ORDER_V - 1] / DenominatorV) -
								  NumeratorV * SumDerivativesV / (DenominatorV * DenominatorV));
						dPdV.z += pVertices[ControlIndexV].z * ((m_WeightsV[VStep] * 
							      m_DerivativeBasisV[V][VStep][PATCH_ORDER_V - 1] / DenominatorV) -
								  NumeratorV * SumDerivativesV / (DenominatorV * DenominatorV));
					}
				}
			...
			D3DXVec3Cross(&Normal, &dPdU, &dPdV);
			...
			D3DXMatrixTranslation(&Translation, VOffset.x, VOffset.y, VOffset.z);

			D3DXMATRIX  Rotation;
			D3DXVECTOR3 Axis;
			D3DXVECTOR3 BaseDirection(0.0f, 1.0f, 0.0f);

			float Angle = acos(D3DXVec3Dot(&BaseDirection, &dPdV));
			D3DXVec3Cross(&Axis, &BaseDirection, &dPdV);
...
{
	for (long Index = 0; Index < NUM_U_POINTS; Index++)
	{
		if (Index % 2)
			m_WeightsU[Index] = 0.707f;
		else
			m_WeightsU[Index] = 1.0f;
	}
	for (Index = 0; Index < NUM_V_POINTS; Index++)
	{
		m_WeightsV[Index] = 1.0f;
	}
}
