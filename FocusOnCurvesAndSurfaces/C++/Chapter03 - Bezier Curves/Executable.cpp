...
//CHostApplication.
static CBezierCurveApplication *g_pCurveApplication = NULL; 
...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, INT)
{
	...
	g_pCurveApplication = new CBezierCurveApplication();

	//Run until the application returns from Go!
    g_pCurveApplication->Go();

	//Clean up the application
	delete (g_pCurveApplication);
							  ...
							  WPARAM wParam, LPARAM lParam)
{
	...
	return g_pCurveApplication->MessageHandler(hWnd, Message, 
											   wParam, lParam);
}
