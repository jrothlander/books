// PolyPatchApplication.cpp: implementation of the CBSplinePatchApplication class.
//
//////////////////////////////////////////////////////////////////////

#include "BSplinePatchApplication.h"
#include <math.h>
...
CBSplinePatchApplication::CBSplinePatchApplication()
{
}
...
BOOL CBSplinePatchApplication::PostInitialize()
{	
...
void CBSplinePatchApplication::SetKnotVectors()
{
...
void CBSplinePatchApplication::Render()
{
...
BOOL CBSplinePatchApplication::PreTerminate()
{
...
void CBSplinePatchApplication::DefineBasisFunctions()
{
...
BOOL CBSplinePatchApplication::FillPatchBuffer()
{
	...
	long MidPoint = (NUM_V_POINTS * NUM_U_POINTS / 2);

	pVertices[MidPoint].y = 300.0f + ANIMATE(200.0f);
		...
		for (long V = 0; V < NUM_PATCH_VERTICES; V++)
		{
			...
			for (long UStep = 0; UStep < NUM_U_POINTS; UStep++)
			{
				for (long VStep = 0; VStep < NUM_V_POINTS; VStep++)
				{
					...
					{
						pVertices[Current].x += pVertices[UStep * NUM_V_POINTS + VStep].x * 
												m_BasisFunctionsU[U][UStep][PATCH_ORDER_U - 1] * 
												m_BasisFunctionsV[V][VStep][PATCH_ORDER_V - 1];
						pVertices[Current].y += pVertices[UStep * NUM_V_POINTS + VStep].y * 
												m_BasisFunctionsU[U][UStep][PATCH_ORDER_U - 1] * 
												m_BasisFunctionsV[V][VStep][PATCH_ORDER_V - 1];
						pVertices[Current].z += pVertices[UStep * NUM_V_POINTS + VStep].z * 
												m_BasisFunctionsU[U][UStep][PATCH_ORDER_U - 1] * 
												m_BasisFunctionsV[V][VStep][PATCH_ORDER_V - 1];


						dPdU.x += pVertices[UStep * NUM_V_POINTS + VStep].x * 
								  m_DerivativeBasisU[U][UStep][PATCH_ORDER_U - 1] * 
								  m_BasisFunctionsV[V][VStep][PATCH_ORDER_V - 1];
						dPdU.y += pVertices[UStep * NUM_V_POINTS + VStep].y * 
								  m_DerivativeBasisU[U][UStep][PATCH_ORDER_U - 1] * 
								  m_BasisFunctionsV[V][VStep][PATCH_ORDER_V - 1];
						dPdU.z += pVertices[UStep * NUM_V_POINTS + VStep].z * 
								  m_DerivativeBasisU[U][UStep][PATCH_ORDER_U - 1] * 
								  m_BasisFunctionsV[V][VStep][PATCH_ORDER_V - 1];

						dPdV.x += pVertices[UStep * NUM_V_POINTS + VStep].x * 
												m_BasisFunctionsU[U][UStep][PATCH_ORDER_U - 1] * 
												m_DerivativeBasisV[V][VStep][PATCH_ORDER_V - 1];
						dPdV.y += pVertices[UStep * NUM_V_POINTS + VStep].y * 
												m_BasisFunctionsU[U][UStep][PATCH_ORDER_U - 1] * 
												m_DerivativeBasisV[V][VStep][PATCH_ORDER_V - 1];
						dPdV.z += pVertices[UStep * NUM_V_POINTS + VStep].z * 
												m_BasisFunctionsU[U][UStep][PATCH_ORDER_U - 1] * 
												m_DerivativeBasisV[V][VStep][PATCH_ORDER_V - 1];
					}
				}
...
void CBSplinePatchApplication::DrawBasisFunctions()
{
