...
BOOL CNURBSApplication::FillCurveBuffer()
{
	...
	for (long ControlPoint = 0; ControlPoint < NUM_CONTROL_POINTS; ControlPoint++)
	{
		...
		//DISABLE THE ANIMATED PARTS TO GET A SIMPLE CIRCLE
		if (ControlPoint % 2)
			Radius = 100.0f / cos(TWO_PI / (2.0f * (float)NUM_SIDES)) + ANIMATE(70.0f);
		else
			Radius = 100.0f - ANIMATE(70.0f);
	
		pVertices[ControlPoint].x = 300.0f + Radius * cos((float)ControlPoint * TWO_PI / (2.0f * (float)NUM_SIDES));
		pVertices[ControlPoint].y = 200.0f + Radius * sin((float)ControlPoint * TWO_PI / (2.0f * (float)NUM_SIDES));
...
void CNURBSApplication::SetKnotVector()
{
	...
	for (long i = 3; i < CURVE_ORDER + NUM_CONTROL_POINTS - 3; i += 2)
	{
		m_KnotVector[i] = m_KnotVector[i + 1] = (((float)i - 1.0f) / 2.0f) / (float)NUM_SIDES;
	}

	m_KnotVector[i] = m_KnotVector[i + 1] = m_KnotVector[i + 2] = 1.0f;
}
...
void CNURBSApplication::SetWeights()
{
	...
	{
		if (i % 2)
			m_Weights[i] = cos(TWO_PI / (float)NUM_SIDES / 2.0f);
		else
			m_Weights[i] = 1.0f;
	}
}
