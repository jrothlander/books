...
BOOL CBSplineApplication::FillCurveBuffer()
{
	...
	for (long ControlPoint = 0; ControlPoint < NUM_CONTROL_POINTS; ControlPoint++)
	{
		...
		pVertices[ControlPoint].y = 200.0f + ANIMATE(200.0f * sin(pVertices[ControlPoint].x * 2.5f));
...
void CBSplineApplication::SetKnotVector()
{
	...
	m_MinT = m_KnotVector[CURVE_ORDER - 1];
	m_MaxT = m_KnotVector[NUM_CONTROL_POINTS] - 0.00001f;
}
...
void CBSplineApplication::DefineBasisFunctions()
{
	...
	{
		float t = m_MinT + ((m_MaxT - m_MinT) * (float)Vertex / ((float)NUM_CURVE_VERTICES - 1.0f));

		//force the last compare to work (not really critical for periodic...)
