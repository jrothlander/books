...
BOOL CBSplineApplication::FillCurveBuffer()
{
	...
	pVertices[2].x = 400.0f + ANIMATE(200.0f);
	pVertices[2].y = 0.0f;

	pVertices[3].x = 0.0f;
	pVertices[3].y = 0.0f;

	pVertices[4].x = 200.0f;
	pVertices[4].y = 400.0f;

	pVertices[5].x = 400.0f + ANIMATE(200.0f);
	pVertices[5].y = 0.0f;
...
{
	//This code creates a periodic knot vector
	for (long i = 0; i < NUM_CONTROL_POINTS + CURVE_ORDER; i++)
	{
		m_KnotVector[i] = (float)i / (float)(NUM_CONTROL_POINTS + CURVE_ORDER);
	}

	//offset the bounds slightly to account for floating point errors
	m_MinT = m_KnotVector[CURVE_ORDER - 1] - EPSILON;
	m_MaxT = m_KnotVector[NUM_CONTROL_POINTS] - EPSILON;
}
...
void CBSplineApplication::DefineBasisFunctions()
{
	...
	{
		float t = m_MinT + ((m_MaxT - m_MinT) * (float)Vertex / (float)(NUM_CURVE_VERTICES - 1));
