...
BOOL CNURBSPatchApplication::FillPatchBuffer()
{
	...
	pVertices[1].z = 200.0f;
	pVertices[2].x = 0.0f;
	pVertices[2].z = 200.0f;
	pVertices[3].x = 100.0f;
	pVertices[3].z = 200.0f;
				...
				for (long VStep = 0; VStep < NUM_V_POINTS; VStep++)
				{
						...
						m_KnotVectorV[VStep] <= CurrentV && CurrentV <= m_KnotVectorV[VStep + PATCH_ORDER_V])
					{
						...
						pVertices[Current].x += (NumeratorU * pVertices[ControlIndexU].x / DenominatorU) + 
							                    (NumeratorV * pVertices[ControlIndexV].x / DenominatorV);
						pVertices[Current].y += (NumeratorU * pVertices[ControlIndexU].y / DenominatorU) + 
							                    (NumeratorV * pVertices[ControlIndexV].y / DenominatorV);
						pVertices[Current].z += (NumeratorU * pVertices[ControlIndexU].z / DenominatorU) + 
							                    (NumeratorV * pVertices[ControlIndexV].z / DenominatorV);
